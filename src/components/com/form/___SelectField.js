...
const SelectField = ({ optionValue = "id", autoSelect = false, optionText = "name", defaultValue, defaultValues = {}, onBlur,
    ...
    onSelectItem,filterDefaultValues,
    resource, reference, source, record, form, placeholder, name,
    ...
    useEffect(() => {
        // if (filterDefaultValues && !_.isEmpty(filterDefaultValues))
            getData();
    }, [filterDefaultValues]);
 /*    useEffect(() => {
        if (props.filterDefaultValues && !_.isEqual(props.filterDefaultValues, filterDefaultValues)) {
