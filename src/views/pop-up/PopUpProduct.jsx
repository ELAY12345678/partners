...
const uploadFiles = (files, establishment_id) =>
    new Promise((resolve, reject) => {
        const originFiles = _.map(files, ({ originFileObj }) => originFileObj).filter(Boolean);
...
const columns = ({ onRemove, onEdit }) => [
    ...
    },
    {
        ...
        sorter: true,
        width: 120,
        render: (value) => _.find(DISPLAY_DEVICES, ({ id }) => id === value)?.name || value,
    },
    {
        title: 'Límite',
        ...
        sorter: true,
        width: 100,
        render: (value) => _.find(DISPLAY_LIMITS, ({ id }) => id === value)?.name || value,
    },
    {
        title: 'Imágenes',
        ...
        render: (value) => {
            ...
            return count ? <Tag color="blue">{count} imagen(es)</Tag> : <Tag>Sin imágenes</Tag>;
        },
    },
    {
        title: 'Acciones',
        ...
        render: (id, record) => (
            <Row>
                <Button type="text" onClick={() => onEdit(record)} icon={<AiOutlineEdit />} />
                <AsyncButton
                    type="link"
                    onClick={() => onRemove({ id })}
...
const PopUpProduct = () => {
    const establishment_id = useSelector(
        ({ dashboardReducer }) => dashboardReducer.establishmentFilters?.establishment_id
    );
    ...
    const onRemove = async ({ id }) => {
        await popupsService
            ...
            .catch((error) => message.error('No se pudo eliminar el pop-up! ' + error?.message));
    };
    ...
    const handleSubmit = async (err, values) => {
        ...
        if (pendingFileList.length) {
            try {
                ...
                const uploadedPaths = await uploadFiles(pendingFileList, establishment_id);
                ...
                >
                    ...
                    >
                        ...
                        <Input
                            ...
                            name="sub_title"
                            label="Subtítulo"
                            validations={[{ required: true, message: 'Subtítulo es requerido' }]}
                        />
                        <Select
                            flex={1}
                            name="display_device"
                            label="Dispositivo"
                            size="large"
                            validations={[{ required: true, message: 'Dispositivo es requerido' }]}
                        >
